...
import dwfpy as dwf
from Read_write_reaset.MemristorTest import MemristorCheck
from Configure_Instrument.SetUp_AD2 import SetUp
from Read_write_reaset.ReadMemristor import ReadMemristor
from Read_write_reaset.ResetMemristor import ResetMemristor
from Read_write_reaset.Set_G_Channel import SendWritePulse
...
from Scaling_Factor.Init_Dec_Voltage import Init_Dec_Voltage
...
def main():
    try:
        ...
        # ------------------ MemristorCheck------------------
        if action == "check":
            MemristorCheck(awg_handle, channel) # This function generates triangular pulse of -1.5 V amplitude Drive the memristor to low resistance state
        ...
        if action == "set":
            ...
            for it in range(iterations):
                ...
                # ======================================================
                if DO_Initial_Reset:
                    print("[INIT] Running initial reset...")
                    #ResetMemristor(awg_handle, channel)
                    #time.sleep(0.1) 
                    MemristorCheck(awg_handle, channel) # Triangular pulse of -1.5 V amplitude, Drive the memristor to low resistance state
                else:
                    print("[INIT] Skipping initial MemristorCheck (using current Device state).")
                # Initial conductance, and check if too high
                current_g_value = ReadMemristor(awg_handle, oscilloscope, channel)
                print(f"[INIT] Initial Conductance: {current_g_value * 1e6:.3e} µS, (R={1/current_g_value/1e3:.3e} kΩ) ")
                time.sleep(0.1)
                ...
                while attempts < max_attempts:
                    ...
                    NP_used = int(max(1, round(base_np * K))) # scale number of pulses with K

                    PW_used = cfg["pw"]  # use base pulse width   
                    ...
                    # ---- Send square pulse to memristor ----
                    ...
                    if runaway:
                        ...
                        time.sleep(0.1)
                        current_g_value = ReadMemristor(awg_handle, oscilloscope, channel)
                        print(f"[SAFETY] New Conductance after safety resets: {current_g_value*1e6:.2f} µS")        
