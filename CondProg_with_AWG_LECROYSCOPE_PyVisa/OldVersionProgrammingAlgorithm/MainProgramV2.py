...
def main():
    ...
    try:
        ...
        # Read and increase or decrease the conductance
            ...
            Amplitude_dec = -0.3 # Amplitude in volts for each channel to decrease G
            
            Offset = 0  # Offset in volts  
            Num_Pulses = 100 # Number of pulses to generate for increase the G
            ...
            if len(sys.argv) == 5:
                try:
                    ...
                    for i in range(iterations):
                        ...
                        # Log each scaling factor vs error 
                        log_filename = f"conductance_log_ch{channel}_{intended_g_value*1e6:.0f}uS.csv"
                        with open(log_filename, mode='w', newline='') as log_file:
                            log_writer = csv.writer(log_file)
                            log_writer.writerow(["Attempt", "Direction", "Current_G_µS", "Intended_G_µS", "Scaling_Factor", "Recent_Delta_µS", "Timestamp"])
                        ...
                        while attempts <= max_attempts:
                            ...
                            def calculate_scaling_factor(intended_g_value, current_g_value, max_conductance, conductance_values, attempts, is_increasing=True):
                                error = intended_g_value - current_g_value
                                relative_error = error / intended_g_value
                                absolute_error = abs(error)

                                # Recent change in conductance (for stagnation check)
                                recent_delta = abs(conductance_values[attempts] - conductance_values[attempts - 1]) if attempts > 0 else float('inf')

                                # Increase scaling if memristor shows little response
                                stagnation_penalty = 1.0
                                if recent_delta < 20e-6:  # No significant change in µS
                                    print("Minimal response detected — increasing aggressiveness.")
                                    stagnation_penalty += 0.5 + (attempts / 10)

                                # Main scaling logic
                                scale_base = 1.0 + 4.0 * min(1.0, absolute_error / max_conductance)
                                scaling_factor = scale_base * abs(relative_error) * stagnation_penalty

                                # Clip the result to safe min/max based on direction
                                if is_increasing:
                                    return max(0.1, min(5.0, scaling_factor))
                                else:
                                    return max(0.1, min(3.0, scaling_factor))
                            
                            # Inside your main loop, after computing scaling_factor and before pulse
                            ...
                            if current_g_value < intended_g_value:
                                ...
                                scaling_factor = calculate_scaling_factor(
                                    intended_g_value, current_g_value, max_conductance, conductance_values, attempts, is_increasing=True)
                            ...
                            timestamp = time.time()
                            recent_delta = abs(conductance_values[attempts] - conductance_values[attempts - 1]) if attempts > 0 else 0.0
                            direction = "increase" if current_g_value < intended_g_value else "decrease"
                            ...
                            with open(log_filename, mode='a', newline='') as log_file:
                                ...
                                log_writer.writerow([
                                    attempts,
                                    direction,
                                    round(current_g_value * 1e6, 3),
                                    round(intended_g_value * 1e6, 3),
                                    round(scaling_factor, 3),
                                    round(recent_delta * 1e6, 3),
                                    round(timestamp, 3)
                                ])
