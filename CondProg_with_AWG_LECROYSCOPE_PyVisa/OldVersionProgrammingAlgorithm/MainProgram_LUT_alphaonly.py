...
RESULTS_CSV = r"C:\Users\Junaid\Data\Experimental_data\VMM_and_Con_Programming\ConductanceBoxPlot\programmed_G_results20.csv"
...
def main():
    try:
        ...
        log = ProgLogger(f"logs/run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        ...
        if action == "set":
            ...
            Num_Pulses = 200
            Num_Pulses_dec = 50
            Time_Period = 200e-06  # seconds
            Frequency = 5          # kHz (unused here but preserved)
            max_conductance = 400e-06  # S (400 µS)
            max_attempts = 20
            total_np_increase = 0 # How many pulses memristor takes to increse the G
            ...
            for it in range(iterations):
                ...
                # Tolerance (6-10%), For very low G values i.e< 30. this tolerance should be 0.15 to 0.20
                ...
                additional_reads = 5
                target_reached = True
                DO_STABILITY_CHECK = False   # <-- change to True if you want stability enforced
                
                # Initial scaling parameters
                def init_alpha_beta(intended_g_value, max_conductance, is_increasing=True):
                    """
                    Initialize alpha and beta:
                    - alpha from LUT depending on G_current, G_target, and direction
                    - beta relative to alpha according to β >= α * (Gmax / Gtarget)
                    """

                    # --- LUT for alpha ---
                    alpha = 1.0  # default
                    if is_increasing:
                        if ReadGValue < 50e-06:
                            if intended_g_value <= 50e-06:
                                alpha = 1.5
                        ...
                        elif 50e-06 <= ReadGValue < 150e-06:
                            if intended_g_value <= 150e-06:
                                alpha = 1.5
                        ...
                        elif 150e-06 <= ReadGValue < 250e-06:
                            if intended_g_value <= 250e-06:
                                alpha = 1.5
                            elif intended_g_value <= 350e-06:
                                alpha = 2.5
                            else:
                                alpha = 3.0
                    ...
                    # --- β rule ---
                    #β=α⋅G_"target" /G_"max"  
                    beta = alpha * (max(intended_g_value, 1e-30) / max_conductance)
                ...
                def calculate_scaling_factor(intended_g_value, current_g_value , max_conductance, alpha, beta, is_increasing=True):

                    # If little change, gently increase alpha/beta
                    if len(conductance_values) >= 2 and abs(conductance_values[-1] - conductance_values[-2]) < 10e-6:
                        alpha = alpha + 0.5 + (attempts / max_attempts)
                        beta  = beta  + 0.5 + (attempts / max_attempts)

                    # Compute scaling factor
                    if is_increasing:
                        difference_ratio = (intended_g_value - current_g_value) / max(intended_g_value, 1e-30)
                    else:
                        difference_ratio = (current_g_value - intended_g_value) / max(current_g_value, 1e-30)
                ...
                while attempts <= max_attempts:
                    ...
                    time.sleep(0.010) # little pause after applying pulses
                    ...
                    # Check range and apply stability check
                    if lower_bound <= current_g_value <= upper_bound:
                        ...
                        if target_reached and stable_reads >= additional_reads:
                            print(f"Conductance stable within target range for {additional_reads} reads. Process complete.")
                                # ---- Store successful programming result ----
                            append_success_result(
                                 target_uS=intended_g_value * 1e6,
                                    programmed_uS=current_g_value * 1e6
                                                                        )

                            break
                        else:
                            print("Stability check failed. Reapplying pulses.")
