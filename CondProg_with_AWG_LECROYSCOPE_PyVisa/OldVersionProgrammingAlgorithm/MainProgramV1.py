...
def main():
    ...
    try:
        ...
        # Read and increase or decrease the conductance
            ...
            Amplitude = 0.8 # Amplitude in volts for each channel to increase G
            Amplitude_dec = -0.40 # Amplitude in volts for each channel to decrease G
            
            Offset = 0  # Offset in volts  
            #Num_Pulses = 500 # Number of pulses to generate for increase the G
            Num_Pulses_dec = 100 # Number of pulses to generate for decreaae the G
            pulses_applied=0
            Time_Period = 100e-06 # Time period of the wave
            ...
            if len(sys.argv) == 5:
                try:
                    ...
                    iterations = int(sys.argv[4])
                       ...
                       #******************************************Initial Check Block****************************
                    # PRE-LOOP: Check memristor verbal behavior
                    print("Checking memristor verbal behavior...")
                    test_amplitude = 1.0
                    test_pulses = 500
                    ...
                    time.sleep(0.1)
                    SendWritePulse(awg_handle, test_amplitude, Time_Period, test_pulses, channel)
                    time.sleep(0.2)
                    verbal_g = ReadMemristor(awg_handle,oscilloscope, channel)
                    print(f"Conductance after test pulse: {verbal_g * 1e6:.3e} µS")

                    # Adjust pulse number based on behavior
                    if verbal_g > 300e-6:
                        Num_Pulses = 50
                        print(f"Highly verbal memristor detected. Using {Num_Pulses} pulses.")
                    else:
                        Num_Pulses = 200
                        print(f"Normal memristor behavior. Using {Num_Pulses} pulses.")

                    # Reset after verbal check
                    reset_count = 3
                    for i in range(reset_count):
                        ResetMemristor(awg_handle, channel)
                        time.sleep(0.1)
                        
                        #print(f"Conductance after reset {i + 1}: {g * 1e6:.3e} µS")
                    #******************************************Initial Check Block End****************************     
                    for i in range(iterations):
                        ...
                        tolerance = 0.06 * intended_g_value
                        ...
                        reset_retry_limit = 2 #Useful if memristor is  stuck to a high G value
                        reset_attempts = 0

                        while attempts <= max_attempts:
                            # Append the read value to the conductance_values list
                            conductance_values.append(current_g_value)
                            # ---- Reset check: if too far from target after attempt
                            if current_g_value - intended_g_value > 255e-6:
                                print("Conductance too far from target. Performing resets...")
                                for r in range(3):
                                    ResetMemristor( awg_handle, channel)
                                    time.sleep(0.05)
                                ...
                                # Safety condition: prevent infinite reset loop
                                if reset_attempts >= reset_retry_limit:
                                    print("Memristor conductance stuck at high value. Reset attempts exhausted. Aborting...")
                                    return False  # or break, or raise Exception

                                continue  # Restart loop with updated conductance   
                                
                            # ---- Reset check End : if too far from target after attempt
