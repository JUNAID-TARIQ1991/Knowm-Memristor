...
with dwf.AnalogDiscovery2() as device:
    ...
    for selected_memristor in range(16):
        ...
        # Parametrs
        PulseWidth = 10e-3  # Pulse wdith in second 100 milli
        ...
        pattern[selected_memristor].setup_constant('high', start=True)  # Select memristor
        ...
        wavegen[0].setup(function='triangle', frequency=Frequency,amplitude=Amplitude_ch0, offset=Offset)
        ...
        timer_thread = threading.Timer(0.5, do_trigger)
        timer_thread.start()
     ...
     #   time.sleep(measurement_duration)
        ...
        axs[selected_memristor].plot(voltage_memristor_cathode, linear_func(
            voltage_memristor_cathode, *popt), 'r--', label=f'Fit: R={1/fitted_G/1e3:.4e}')
        axs[selected_memristor].set_xlabel('Voltage [V]')
        axs[selected_memristor].set_ylabel(r'Current [ $\mu $A]')
        ...
        with open('conductance_values.txt', 'a') as f:f.write(f'Memristor {selected_memristor+1}: Conductance = {fitted_G * 1e6:.3e} mu S \t Resistance = {1 /fitted_G/1e3:.3e} kohm  \n')
    ...
    plt.tight_layout()
    plt.savefig('all_memristors_IV_curves.png', dpi=300)
    plt.show() 